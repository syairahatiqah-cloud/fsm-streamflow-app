...
st.set_page_config(page_title="FSM Streamflow App", layout="wide")
st.title("FSM Streamflow Imputation (Upload File Only)")
...
if len(cols) < 2:
    st.error("Your file must have at least 2 columns (datetime + streamflow).")
    st.stop()

time_col = st.selectbox("Select datetime column", cols, index=0)
val_col  = st.selectbox("Select streamflow column", cols, index=1)
...
raw_fig.update_layout(
    title=f"Streamflow Time Series: {val_col}",
...
# ============================================================
# 2) Missing values monthly (download CSV + HTML + PNG)
# ============================================================
...
missing_values_per_month = tmp.groupby(["Year", "Month"])[val_col].apply(lambda x: x.isnull().sum()).reset_index(name="Missing_Count")
total_observations_per_month = tmp.groupby(["Year", "Month"]).size().reset_index(name="Total_Observations_Month")

monthly_summary = pd.merge(missing_values_per_month, total_observations_per_month, on=["Year", "Month"], how="left")
monthly_summary["Missing_Percentage"] = (monthly_summary["Missing_Count"] / monthly_summary["Total_Observations_Month"]) * 100
...
if run:
    ...
    fig2.update_layout(
        title=f"Original vs FSM Imputed: {val_col}",
    ...
    # Monthly seasonality: original vs imputed (Matplotlib)
    ...
    ax.set_title(f"Monthly Seasonality: {val_col} (Original vs FSM Imputed)")
    ax.set_xlabel("Month")
    ...
    # Save matplotlib plot to bytes for download
    buf = io.BytesIO()
    figm.tight_layout()
